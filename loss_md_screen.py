...
class Loss_md_screen(nn.Module):
	def __init__(self, weight_angle=10):
		super(Loss_md_screen, self).__init__()
		self.weight_angle = weight_angle

	def forward(self, gt_score, pred_score, ignored_map):
		if torch.sum(gt_score) < 1:
			return torch.sumpred_score * 0
		...
		# iou_loss = torch.sum(iou_loss_map * gt_score) / torch.sum(gt_score)
		# geo_loss = self.weight_angle * angle_loss + iou_loss
		print('classify loss is {:.8f}'.format(classify_loss))
